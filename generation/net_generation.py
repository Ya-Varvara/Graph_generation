...
from generation.algorithms import random_flow, FordFulkerson
    ...
    for x in graph:
        for y in graph[x]:
            if (x, y) in cut_edges:
                continue
            f = graph[x][y]
            add = randint(1, max_weight - f + 1) if min_weight <= f < max_weight else randint(min_weight - f + 1, max_weight - f + 1)
            graph[x][y] += add
    return
